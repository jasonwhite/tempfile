...
fn test_make_uds_conflict() {
    ...
    // This should always be at least NTHREADS.
    ...
    let sockets: Vec<_> = threads.into_iter().map(|thread| thread.join().unwrap().unwrap()).collect();

    // Number of tries is exactly equal to (n*(n+1))/2.
    assert_eq!(tries.load(Ordering::Relaxed), (NTHREADS * (NTHREADS + 1)) / 2);
